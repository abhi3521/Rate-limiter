...
public class Main {
    public static void main(String[] args) {
        RateLimiter<Integer, Integer> limiter = new RateLimiter<>(10, 10); // Allow 10 calls within a 10 sec window
        ...
        for (int i = 0; i < 15; i++) {
            try {
                System.out.println(rateLimitedSquare.apply(i));
            } catch (RateLimiter.RateLimitExceededException e) {
                System.out.println("Rate limit exceeded for input: " + i);
            }
        }
    }
}
...
/*
Rate Limiter
A rate limiter is used to restrict the rate at which a resource is accessed. The requirement is to
implement a Rate Limiter that can wrap any function call and prevent calls in excess of the specified
limit. First a rate limiter is configured with the allowed rate limit. as shown
